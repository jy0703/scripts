...
[Script]
中国移动签到获取Cookie = type=http-request,pattern=https:\/\/wx\.10086\.cn\/qwhdhub\/qwhdmark\/,requires-body=1,max-size=0,binary-body-mode=0,timeout=30,script-path=https://raw.githubusercontent.com/jy0703/scripts/main/scripts/cmcc_sign.js,script-update-interval=0

中国移动签到 = type=cron,cronexp="0 9 * * *",timeout=60,script-path=https://raw.githubusercontent.com/jy0703/scripts/main/scripts/cmcc_sign.js,script-update-interval=0
...
[Script]
http-request https:\/\/wx\.10086\.cn\/qwhdhub\/qwhdmark\/ tag=中国移动签到获取Cookie,script-path=https://raw.githubusercontent.com/jy0703/scripts/main/scripts/cmcc_sign.js,requires-body=1

cron "0 9 * * *" script-path=https://raw.githubusercontent.com/jy0703/scripts/main/scripts/cmcc_sign.js,tag=中国移动签到,enable=true
...
[rewrite_local]
https:\/\/wx\.10086\.cn\/qwhdhub\/qwhdmark\/ url script-request-header https://raw.githubusercontent.com/jy0703/scripts/main/scripts/cmcc_sign.js

[task_local]
0 9 * * * https://raw.githubusercontent.com/jy0703/scripts/main/scripts/cmcc_sign.js, tag=中国移动签到, img-url=https://raw.githubusercontent.com/Orz-3/mini/master/Color/cmcc.png, enabled=true
...
http:
  mitm:
    - "wx.10086.cn"
  script:
    - match: https:\/\/wx\.10086\.cn\/qwhdhub\/qwhdmark\/
      name: 中国移动签到获取Cookie
      type: request
      require-body: true
...
script-providers:
  中国移动签到:
    url: https://raw.githubusercontent.com/jy0703/scripts/main/scripts/cmcc_sign.js
    interval: 86400

 */

const $ = new Env('中国移动签到');
$.is_debug = getEnv('is_debug') || 'false';  // 调试模式
$.userInfo = getEnv('CMCC_COOKIE') || '';  // 获取账号
...
async function main() {
    if ($.userArr.length) {
        ...
        throw new Error('未找到 CMCC_COOKIE 变量 ❌');
    }
}
...
function GetCookie() {
    try {
        ...
        // 检查是否是获取mark接口的请求
        if ($request.url && $request.url.includes('/qwhdhub/api/mark/do/mark')) {
            // 从请求头中获取Cookie
            const cookieHeader = $request.headers['Cookie'] || $request.headers['cookie'];
            
            if (cookieHeader) {
                // 使用 find() 方法找到与 cookie 匹配的 token，以新增/更新用户 cookie
                const cookieList = $.userInfo.split(/&|\n/).filter(t => t.trim());
                const cookieExists = cookieList.some(cookie => cookie.trim() === cookieHeader.trim());
                
                if (!cookieExists) {
                    msg += `🆕 新增用户 Cookie: ${cookieHeader}`;
                    cookieList.push(cookieHeader);
                    // 写入数据持久化
                    $.setdata(cookieList.join('&'), 'CMCC_COOKIE');
                } else {
                    msg += `✅ Cookie 已存在: ${cookieHeader}`;
                }
                
                $.Messages.push(msg), $.log(msg);
            } else {
                $.log("❌ 未找到 Cookie header");
            }
        } else {
            // 如果请求URL不包含指定路径，则直接返回
            return;
        }
    } catch (e) {
...
async function doMark(cookie) {
    ...
    try {
        ...
        const options = {
            ...
            headers: {
                'User-Agent': 'Mozilla/5.0 (iPhone; CPU iPhone OS 18_7 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148/wkwebview leadeon/12.0.1/CMCCIT',
                ...
                'Referer': 'https://wx.10086.cn/qwhdhub/qwhdmark/1021122301?channelId=P00000057578&yx=1099157070&token=QWHDSSO0108232910799336882300057578yx1099157070',
                'login-check': '1',
                'Sec-Fetch-Dest': 'empty',
                'Cookie': cookie
            },
            body: '{}'  // 修改为空JSON字符串而不是空对象
        }
        ...
        if (result?.success) {
            ...
            // 根据返回结果构建奖励信息
            if (prize) {
                const prizeInfo = `${prize.prizeName}(${prize.prizeValue}${prize.categoryOverview?.unit || ''})`;
                msg += `\n🎁 获得奖励: ${prizeInfo}`;
                $.beforeMsgs = `\n签到时间: ${prize.winAt}\n奖励名称: ${prize.prizeName}\n奖励价值: ${prize.prizeValue}${prize.categoryOverview?.unit || ''}\n奖励类别: ${prize.prizeCategory}\n`;
            }
        } else if (result?.msg?.includes('已签') || result?.message?.includes('今日已签')) {
            msg += `签到: 📝 今日已签到`;
        } else {
    ...
    try {
        ...
        const _http = [
            ...
            new Promise((resolve, reject) => {
                ...
                // 如果是POST请求且body不是字符串，则将其转换为字符串
                if ((_method === 'post' || _method === 'POST') && options.body && typeof options.body !== 'string') {
