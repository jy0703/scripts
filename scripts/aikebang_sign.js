...
(function (h) {
    for (var s = CryptoJS, f = s.lib, g = f.WordArray, m = f.Hasher, r = [], q = 0; 64 > q; q++) r[q] = 4294967296 * h.abs(h.sin(q + 1)) | 0;
    f = m.extend({
        _doReset: function () {
    ...
    s.MD5 = m._createHelper(f);
    s.HmacMD5 = m._createHmacHelper(f)
})(Math);